...
export default function GitTools() {
  const { data, isLoading, isError, error } = useEngineTools('git');
      ...
      <Grid size={{ xs: 12, md: 4 }}>
        <Paper sx={{ p: 1, height: PANEL_HEIGHT, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
            <Typography variant="subtitle1" sx={{ fontSize: 12 }}>Git Tools</Typography>
          </Stack>
          {isLoading && <LinearProgress />}
          {isError && <Alert severity="error">{(error as any)?.message || 'Failed to load tools'}</Alert>}
          <TextField id="git-filter" name="gitFilter" fullWidth size="small" placeholder="Search tools..." value={filter} onChange={(e)=>setFilter(e.target.value)} sx={{ mb: 1 }} />
