  ...
  module Transports
    ...
    class WebSocket
      ...
      def handle_client(socket, dispatcher, log)
        ...
        begin
          request_line, headers = read_http_request(socket)
          unless request_line && headers['upgrade']&.downcase == 'websocket'
            write_http_response(socket, 400, 'Bad Request', 'WebSocket upgrade required')
            return
          end
          # Path check
          ...
          if !key || key.empty?
            write_http_response(socket, 400, 'Bad Request', 'Missing Sec-WebSocket-Key')
            return
          end
