...
module Savant
  module Context
    ...
    class FTS
      # Initialize with a DB connection wrapper. Accepts a Savant::Framework::DB or any
      ...
      def search(q:, repo: nil, limit: 10, memory_only: false)
        ...
        if memory_only
          ...
          clauses = placeholders.map { |ph| "f.rel_path ILIKE #{ph}" }.join(' OR ')
          where_mb = " AND (#{clauses})"
        end
