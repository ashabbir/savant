...
module Savant
  module Jira
    # Builder for Jira registrar with middlewares and tools.
    class RegistrarBuilder
      def self.build(engine)
        new(engine).build
      end

      def initialize(engine)
        @engine = engine || Savant::Jira::Engine.new
      end

      def build
        builder = Savant::Framework::MCP::Core::DSL::Builder.new
        apply_middlewares(builder)
        register_tools(builder)
        builder.registrar
      end
      ...
      def apply_middlewares(builder)
        ...
        validate_mw = lambda do |ctx, name, a, nxt|
          schema = ctx[:schema]
          a2 = begin
        ...
        builder.middleware(&log_mw)
        builder.middleware(&validate_mw)
      ...
      def register_tools(builder)
        register_search_and_fetch(builder)
        register_issue_crud(builder)
        register_transitions_and_links(builder)
        register_attachments_and_watchers(builder)
        register_misc(builder)
      end

      def register_search_and_fetch(builder)
        builder.tool 'jira_search', description: 'Run a Jira JQL search',
        ...
        builder.tool 'jira_get_issue', description: 'Get a Jira issue (v3)',
      ...
      def register_issue_crud(builder)
        builder.tool 'jira_create_issue', description: 'Create a Jira issue (v3)',
                                          schema: {
                                            type: 'object',
                                            properties: {
                                            ...
                                            required: %w[projectKey summary issuetype]
                                          } do |_ctx, a|
        ...
        builder.tool 'jira_update_issue', description: 'Update a Jira issue (v3)',
        ...
        builder.tool 'jira_delete_issue', description: 'Delete issue (v3)',
                                          schema: { type: 'object', properties: { key: { type: 'string' } }, required: ['key'] } do |_ctx, a|
          @engine.delete_issue(key: a['key'])
        end
      end

      def register_transitions_and_links(builder)
        builder.tool 'jira_transition_issue', description: 'Transition a Jira issue (v3)',
        ...
        builder.tool 'jira_link_issues', description: 'Link two issues (v3)',
      ...
      def register_attachments_and_watchers(builder)
        builder.tool 'jira_add_comment', description: 'Add a comment to an issue (v3)',
        ...
        builder.tool 'jira_delete_comment', description: 'Delete a comment (v3)',
        ...
        builder.tool 'jira_download_attachments', description: 'Download issue attachments (v3)',
                                                  schema: { type: 'object', properties: { key: { type: 'string' } }, required: ['key'] } do |_ctx, a|
          @engine.download_attachments(key: a['key'])
        end

        builder.tool 'jira_add_attachment', description: 'Upload attachment to issue (v3)',
        ...
        builder.tool 'jira_add_watcher_to_issue', description: 'Add watcher (v3)',
        ...
        builder.tool 'jira_assign_issue', description: 'Assign issue (v3)',
        ...
        builder.tool 'jira_bulk_create_issues', description: 'Bulk create issues (v3)',
      ...
      def register_misc(builder)
        builder.tool 'jira_self', description: 'Verify Jira credentials',
    ...
    module Tools
      ...
      def build_registrar(engine = nil)
        Savant::Jira::RegistrarBuilder.build(engine)
      end
    end
  end
end
