...
module Savant
  module Jira
    ...
    class RegistrarBuilder
      ...
      def build
        build_var = Savant::MCP::Core::DSL::Builder.new
        apply_middlewares(build_var)
        register_tools(build_var)
        build_var.registrar
      end

      def apply_middlewares(b)
        ...
        validate_mw = lambda do |ctx, _nm, a, nxt|
          schema = ctx[:schema]
          a2 = begin
        ...
        b.middleware(&log_mw)
        b.middleware(&validate_mw)

        # Minimal logging
        b.middleware do |ctx, nm, a, nxt|
          start = Process.clock_gettime(Process::CLOCK_MONOTONIC)
          out = nxt.call(ctx, nm, a)
          dur_ms = ((Process.clock_gettime(Process::CLOCK_MONOTONIC) - start) * 1000).round
          begin
      ...
      def register_tools(b)
        register_search_and_fetch(b)
        register_issue_crud(b)
        register_transitions_and_links(b)
        register_attachments_and_watchers(b)
        register_misc(b)
      end

      def register_search_and_fetch(b)
        b.tool 'jira_search', description: 'Run a Jira JQL search',
        ...
        b.tool 'jira_get_issue', description: 'Get a Jira issue (v3)',
      ...
      def register_issue_crud(b)
        b.tool 'jira_create_issue', description: 'Create a Jira issue (v3)',
        ...
        b.tool 'jira_update_issue', description: 'Update a Jira issue (v3)',
        ...
        b.tool 'jira_delete_issue', description: 'Delete issue (v3)',
                                    schema: { type: 'object', properties: { key: { type: 'string' } }, required: ['key'] } do |_ctx, a|
          @engine.delete_issue(key: a['key'])
        end
      end

      def register_transitions_and_links(b)
        b.tool 'jira_transition_issue', description: 'Transition a Jira issue (v3)',
        ...
        b.tool 'jira_link_issues', description: 'Link two issues (v3)',
      ...
      def register_attachments_and_watchers(b)
        b.tool 'jira_add_comment', description: 'Add a comment to an issue (v3)',
        ...
        b.tool 'jira_delete_comment', description: 'Delete a comment (v3)',
        ...
        b.tool 'jira_download_attachments', description: 'Download issue attachments (v3)',
                                            schema: { type: 'object', properties: { key: { type: 'string' } }, required: ['key'] } do |_ctx, a|
          @engine.download_attachments(key: a['key'])
        end

        b.tool 'jira_add_attachment', description: 'Upload attachment to issue (v3)',
        ...
        b.tool 'jira_add_watcher_to_issue', description: 'Add watcher (v3)',
        ...
        b.tool 'jira_assign_issue', description: 'Assign issue (v3)',
        ...
        b.tool 'jira_bulk_create_issues', description: 'Bulk create issues (v3)',
      ...
      def register_misc(b)
        b.tool 'jira_self', description: 'Verify Jira credentials',
