...
- Params:
  - `workflow` (string, required): workflow ID (e.g., `cr_scope`, `cr_quality_security`, `cr_delivery`).
  - `params` (object, optional): recommended keys for code review workflows:
...
- Example (cr_scope):
  ...
  ruby ./bin/savant call 'think.plan' --service=think \
    --input='{"workflow":"cr_scope","params":{"ticket_id":"ABC-123","repo_path":".","branch":"feature","base_branch":"main","pr_number":42,"date":"2025-11-19","reviewer":"alice"},"run_id":"cr-scope-001","start_fresh":true}'
  ```
...
- Example:
  ...
  ruby ./bin/savant call 'think.next' --service=think \
    --input='{"workflow":"cr_scope","run_id":"cr-scope-001","step_id":"pr_snapshot","result_snapshot":{"title":"...","files":["lib/foo.rb"],"labels":["backend"]}}'
  ```
...
- Run the provided workflows:
  - Code review (independent, global): `cr_scope`, `cr_quality_security`, `cr_delivery`
  - Other: `develop_ticket_v1`, `ticket_grooming_v1`
...
> Note: The legacy `code_review_initial` and `code_review_final` are deprecated and replaced by the three independent workflows above.
...
- Gating: Steps can include `applicable_when` to auto‑skip inapplicable work based on prior captured results. Schema: `{ var, field?, equals=true, min_conf_field?, min_conf? }`.
...
- Diffs/commits: use GitHub MCP (e.g., `github.pr.snapshot`, `github.ci.status`); do not use `git diff` directly.
- Cross‑repo impact: use Context MCP `fts/search` for dependent repository references.
- Output reports: `code-reviews/<ticket_id>/<YYYY-MM-DD>/<review_type>.md` with Mermaid visuals embedded in the markdown (no external assets directory).
