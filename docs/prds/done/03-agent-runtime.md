...
Note: This PRD describes an earlier design that used an SLM-first approach with LLM escalation. The current runtime uses the Redis Reasoning Worker for decisions without local SLM fallback. See memory_bank/runtime.md for the current architecture.
